...
def modificarPeliculas():
    ...
    conexionBD=conectar()
    if conexionBD!=None:
        nombre1=input("Dame el nombre de la pelicula a modificar: ").upper().strip()
        cursor=conexionBD.cursor()
        sql="select * from peliculas where nombre=%s"
        val=(nombre1,)
        cursor.execute(sql,val)
        ...
        if registros:
          ...
          if resp=="si":
            ...
            val=(pelicula["nombre"],pelicula["categoria"],pelicula["clasificacion"],pelicula["genero"],pelicula["idioma"],nombre1)
            cursor.execute(sql,val)
            conexionBD.commit()
            print("\n\t\t :::¡LA OPERACION SE REALIZÓ CON EXÍTO! :::")
        else:
